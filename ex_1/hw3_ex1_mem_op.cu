...
__global__ void gpu_grayscale(int width, int height, float *image, float *image_out)
{
    ...
    {
        int offset_out = yIdx * width;
        int offset = offset_out * 3;

        float *pixel = &image[offset + xIdx * 3];
        // Convert to grayscale following the "luminance" model
        image_out[offset_out + xIdx] = pixel[0] * 0.0722f + // B
                                       pixel[1] * 0.7152f + // G
                                       pixel[2] * 0.2126f;  // R  
    } 
}
...
__global__ void gpu_gaussian(int width, int height, float *image, float *image_out)
{
    ...
    int index_sh = threadIdx.x * BLOCK_SIZE_SH + threadIdx.y;
    ...
    sh_block[index_sh] = image[offset];
    __syncthreads();
    
    if (index_x < (width - 1))
        sh_block[index_sh + 1] = image[offset + 1];
    __syncthreads();
    if (index_x < (width - 2))
        sh_block[index_sh + 2] = image[offset + 2];
    __syncthreads();
    if (index_y < (height - 1))
        sh_block[index_sh + BLOCK_SIZE_SH] = image[offset + width];
    __syncthreads();
    if (index_y < (height - 2))
        sh_block[index_sh + 2 * BLOCK_SIZE_SH] = image[offset + 2 * width];
    __syncthreads();
    if (index_x < (width - 1) && index_y < (height - 1))
        sh_block[index_sh + BLOCK_SIZE_SH + 1] = image[offset + width + 1];
    __syncthreads();
    if (index_x < (width - 2) && index_y < (height - 2))
        sh_block[index_sh + 2 * BLOCK_SIZE_SH + 2] = image[offset + 2 * width + 2];
    __syncthreads();

    if (index_x < (width - 2) && index_y < (height - 2))
    {
        image_out[offset] = applyFilter(&sh_block[index_sh],
...
__global__ void gpu_sobel(int width, int height, float *image, float *image_out)
{
    ...
    if (index_x < (width - 2) && index_y < (height - 2))
    {
        float gx = applyFilter(&sh_block[index_sh],
            BLOCK_SIZE_SH, sobel_x, 3);

        float gy = applyFilter(&sh_block[index_sh],
            BLOCK_SIZE_SH, sobel_y, 3);

        image_out[offset] = sqrt(gx * gx + gy * gy);
    }
